...
function ensureAudio(){
  if(!audioCtx){
    ...
    try{audioCtx=new AC({latencyHint:'interactive'})}catch{audioCtx=new AC()}master=audioCtx.createGain();master.gain.value=.82;
...
socket.on('connect',()=>{updateConn(true);refreshRooms();if(roomState&&micOn)socket.emit('voice:state',{voiceOn:true});if(roomState&&wasDisconnected&&!intentionalLeave)rejoinCurrentRoom()});socket.on('disconnect',()=>{updateConn(false);wasDisconnected=true;if(roomState)toast('Connection lost. Reconnecting…')});socket.on('connect_error',()=>updateConn(false));updateConn(socket.connected);
socket.on('lobby:update',d=>{publicRooms=d?.rooms||[];renderLobbyRooms()});
fetch('/config',{cache:'no-store'}).then(r=>r.json()).then(d=>{appSettings={...appSettings,...(d.settings||{})};sfxOn=true;renderAudioButtons()}).catch(()=>{});
...
function enterRoom(room,messages=[]){roomState=room;intentionalLeave=false;wasDisconnected=false;landing.classList.add('hidden');gameShell.classList.remove('hidden');$('chatFab')?.classList.remove('hidden');$('roomPanelToggle')?.classList.remove('hidden');visualPositions.clear();room.players.forEach(p=>visualPositions.set(p.id,p.position||1));setChatHistory(messages);setChatOpen(false);setRoomPanelOpen(false);renderAll();sfx('join');history.replaceState({},'',room.path||'/snakes');setTimeout(()=>startLocalVoice(true),80)}
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('/service-worker.js?v=12.0.0',{updateViaCache:'none'});await reg.update()}catch{}})}
...
function renderPlayers(){
  if(!roomState)return;
  const max=roomState.settings?.maxPlayers||appSettings.maxPlayers;
  playerCount.textContent=`${roomState.players.length}/${max}`;
  const compactCount=$('roomPanelCount');if(compactCount)compactCount.textContent=`${roomState.players.length}/${max}`;
...
gameNav.addEventListener('click',async e=>{const b=e.target.closest('button[data-game]');if(!b||!roomState)return;if(socket.id!==roomState.hostId)return toast('Only the host can switch games.');const r=await ackEmit('game:select',{game:b.dataset.game});if(!r.ok)toast(r.error||'Could not switch game');else setRoomPanelOpen(false)});

// ---------- COLLAPSIBLE ROOM / GAME CONTROLS ----------
const roomPanel=$('roomPanel'),roomPanelToggle=$('roomPanelToggle'),roomPanelBackdrop=$('roomPanelBackdrop');
function roomPanelIsOpen(){return !!roomPanel?.classList.contains('open')}
function setRoomPanelOpen(open){if(!roomPanel)return;if(open&&typeof chatIsVisible==='function'&&chatIsVisible())setChatOpen(false);roomPanel.classList.toggle('open',!!open);roomPanelBackdrop?.classList.toggle('open',!!open);roomPanel.setAttribute('aria-hidden',open?'false':'true');roomPanelToggle?.setAttribute('aria-expanded',open?'true':'false');roomPanelToggle?.classList.toggle('open',!!open)}
roomPanelToggle?.addEventListener('click',()=>setRoomPanelOpen(!roomPanelIsOpen()));roomPanelBackdrop?.addEventListener('click',()=>setRoomPanelOpen(false));

// ---------- LIVE CHAT POPUP ----------
const chatDrawer=$('chatDrawer'),chatBackdrop=$('chatBackdrop'),chatMessages=$('chatMessages'),chatForm=$('chatForm'),chatInput=$('chatInput'),chatUnread=$('chatUnread');
const chatFab=$('chatFab'),chatCloseBtn=$('chatCloseBtn'),chatPeek=$('chatPeek');
let chatItems=[],unreadChat=0,chatPeekTimer=null;
...
function setChatHistory(items=[]){chatItems=Array.isArray(items)?items.slice(-80):[];unreadChat=0;renderChat();updateChatUnread();hideChatPeek()}
...
function chatIsVisible(){return !!chatDrawer?.classList.contains('open')}
function hideChatPeek(){clearTimeout(chatPeekTimer);chatPeekTimer=null;chatPeek?.classList.add('hidden');chatPeek?.classList.remove('show')}
function showChatPeek(m){if(!chatPeek||chatIsVisible())return;chatPeek.querySelector('b').textContent=m.from||'Player';chatPeek.querySelector('span').textContent=m.text;chatPeek.classList.remove('hidden');requestAnimationFrame(()=>chatPeek.classList.add('show'));clearTimeout(chatPeekTimer);chatPeekTimer=setTimeout(hideChatPeek,4800)}
function setChatOpen(open){if(!chatDrawer)return;chatDrawer.classList.toggle('open',!!open);chatBackdrop?.classList.toggle('open',!!open);chatDrawer.setAttribute('aria-hidden',open?'false':'true');chatFab?.setAttribute('aria-expanded',open?'true':'false');if(open){hideChatPeek();unreadChat=0;updateChatUnread();setRoomPanelOpen(false);setTimeout(()=>{chatMessages.scrollTop=chatMessages.scrollHeight;chatInput?.focus()},100)}}
...
socket.on('chat:message',m=>{chatItems.push(m);if(chatItems.length>80)chatItems.shift();if(m.playerId!==socket.id&&!chatIsVisible()){unreadChat++;updateChatUnread();showChatPeek(m);sfx('join')}renderChat()});
chatFab?.addEventListener('click',()=>setChatOpen(!chatIsVisible()));chatCloseBtn?.addEventListener('click',()=>setChatOpen(false));chatBackdrop?.addEventListener('click',()=>setChatOpen(false));chatPeek?.addEventListener('click',()=>setChatOpen(true));
document.addEventListener('keydown',e=>{if(e.key==='Escape'){if(chatIsVisible())setChatOpen(false);else if(roomPanelIsOpen())setRoomPanelOpen(false)}});

// ---------- MEME STICKERS ----------
function renderStickers(){const tray=$('stickerTray');tray.innerHTML='';stickerList.forEach(st=>{const b=document.createElement('button');b.className='sticker-btn';b.type='button';b.title=st.name;const im=document.createElement('img');im.src=st.url;im.alt=st.name;im.loading='lazy';im.decoding='async';im.style.objectPosition=st.position||'center';im.addEventListener('error',()=>{b.classList.add('broken');im.remove()},{once:true});b.appendChild(im);const label=document.createElement('small');label.textContent=st.name;b.append(label);b.addEventListener('click',async()=>{ensureAudio();const r=await ackEmit('sticker:send',{id:st.id});if(!r.ok&&r.error!=='Too fast.')toast(r.error||'Could not send')});tray.appendChild(b)})}
let stickerTimer=null;socket.on('sticker:pop',data=>{const pop=$('stickerPopup'),im=pop.querySelector('img');clearTimeout(stickerTimer);im.src=data.url;im.classList.remove('hidden');pop.querySelector('b').textContent=data.name;pop.querySelector('small').textContent=`${data.from} reacted`;pop.classList.remove('hidden');sfx('sticker');stickerTimer=setTimeout(()=>pop.classList.add('hidden'),data.ms||3000)});
...
// ---------- LOW-LATENCY LIVE VOICE (SOCKET PCM V2) ----------
let localStream=null,micOn=false,voiceCaptureNode=null,voiceSourceNode=null,voiceSilentGain=null,voiceSeq=0,voiceStarting=false;
let voiceOutput=null,voiceCompressor=null;const voiceNextTime=new Map(),voiceLastSeq=new Map();const VOICE_RATE=16000;
function renderAudioButtons(){const mb=$('micBtn');if(mb){mb.classList.toggle('on',micOn);mb.querySelector('b').textContent=micOn?'VOICE LIVE':'VOICE OFF';mb.title=micOn?'Your microphone is live':'Turn on live voice'}const sb=$('speakerBtn');if(sb){sb.classList.toggle('on',speakerOn);sb.querySelector('b').textContent=speakerOn?'SPEAKER ON':'SPEAKER OFF'}if(voiceOutput)voiceOutput.gain.value=speakerOn ? .94 : 0}
function ensureVoiceOutput(){ensureAudio();if(!audioCtx)return null;if(!voiceOutput){voiceOutput=audioCtx.createGain();voiceOutput.gain.value=speakerOn ? .94 : 0;voiceCompressor=audioCtx.createDynamicsCompressor();voiceCompressor.threshold.value=-22;voiceCompressor.knee.value=14;voiceCompressor.ratio.value=4;voiceCompressor.attack.value=.003;voiceCompressor.release.value=.12;voiceOutput.connect(voiceCompressor);voiceCompressor.connect(audioCtx.destination)}return voiceOutput}
function downsampleToInt16(input,inputRate,targetRate=VOICE_RATE){if(!input?.length)return new Int16Array(0);if(inputRate<=targetRate){const out=new Int16Array(input.length);for(let i=0;i<input.length;i++){const x=Math.max(-1,Math.min(1,input[i]));out[i]=x<0?x*32768:x*32767}return out}const ratio=inputRate/targetRate,outLen=Math.max(1,Math.floor(input.length/ratio)),out=new Int16Array(outLen);let pos=0;for(let i=0;i<outLen;i++){const next=Math.min(input.length,Math.floor((i+1)*ratio));let sum=0,count=0;for(;pos<next;pos++){sum+=input[pos];count++}const x=Math.max(-1,Math.min(1,count?sum/count:0));out[i]=x<0?x*32768:x*32767}return out}
function sendVoicePcm(pcm){if(!micOn||!roomState||!socket.connected||!pcm?.byteLength)return;const tx=typeof socket.compress==='function'?socket.compress(false):socket;tx.emit('voice:chunk',{pcm:pcm.buffer,sampleRate:VOICE_RATE,seq:++voiceSeq})}
async function buildVoiceCapture(){ensureAudio();if(!audioCtx||!localStream)return false;try{await audioCtx.resume()}catch{}voiceSourceNode=audioCtx.createMediaStreamSource(localStream);voiceSilentGain=audioCtx.createGain();voiceSilentGain.gain.value=0;voiceSilentGain.connect(audioCtx.destination);
  if(audioCtx.audioWorklet&&window.AudioWorkletNode){try{await audioCtx.audioWorklet.addModule('/voice-worklet-v12.js?v=12.0.0');const node=new AudioWorkletNode(audioCtx,'playverse-voice-capture',{numberOfInputs:1,numberOfOutputs:1,outputChannelCount:[1]});node.port.onmessage=e=>{const pcm=e.data?.pcm;if(pcm)sendVoicePcm(new Int16Array(pcm))};voiceSourceNode.connect(node);node.connect(voiceSilentGain);voiceCaptureNode=node;return true}catch(e){console.warn('AudioWorklet unavailable, using fallback',e)}}
  const node=audioCtx.createScriptProcessor?.(2048,1,1);if(!node)return false;node.onaudioprocess=e=>sendVoicePcm(downsampleToInt16(e.inputBuffer.getChannelData(0),audioCtx.sampleRate));voiceSourceNode.connect(node);node.connect(voiceSilentGain);voiceCaptureNode=node;return true;
}
function teardownVoiceCapture(){try{voiceSourceNode?.disconnect()}catch{}try{voiceCaptureNode?.disconnect()}catch{}try{voiceSilentGain?.disconnect()}catch{}if(voiceCaptureNode&&'onaudioprocess'in voiceCaptureNode)voiceCaptureNode.onaudioprocess=null;voiceSourceNode=null;voiceCaptureNode=null;voiceSilentGain=null;voiceNextTime.clear();voiceLastSeq.clear()}
async function startLocalVoice(auto=false){if(micOn||voiceStarting)return true;if(!navigator.mediaDevices?.getUserMedia){if(!auto)toast(location.protocol==='https:'?'Voice is not supported in this browser.':'Voice needs HTTPS to use the microphone.');return false}voiceStarting=true;try{ensureAudio();try{await audioCtx?.resume()}catch{}localStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,channelCount:{ideal:1},sampleRate:{ideal:48000},latency:{ideal:.02}},video:false});const track=localStream.getAudioTracks()[0];const ok=await buildVoiceCapture();if(!ok)throw new Error('Audio capture unavailable');micOn=true;voiceSeq=0;track?.addEventListener('ended',()=>{if(micOn&&roomState&&!intentionalLeave){micOn=false;teardownVoiceCapture();renderAudioButtons();setTimeout(()=>startLocalVoice(true),500)}},{once:true});socket.emit('voice:state',{voiceOn:true});renderAudioButtons();if(!auto)toast('Live voice connected.');return true}catch(e){localStream?.getTracks().forEach(t=>t.stop());localStream=null;teardownVoiceCapture();micOn=false;renderAudioButtons();if(!auto||e?.name!=='NotAllowedError')toast('Microphone blocked. Tap VOICE OFF and allow microphone.');return false}finally{voiceStarting=false}}
function stopLocalVoice(){micOn=false;teardownVoiceCapture();localStream?.getTracks().forEach(t=>t.stop());localStream=null;if(socket.connected&&roomState)socket.emit('voice:state',{voiceOn:false});renderAudioButtons()}
function pcmArray(data){if(data instanceof ArrayBuffer)return new Int16Array(data);if(ArrayBuffer.isView(data))return new Int16Array(data.buffer,data.byteOffset,Math.floor(data.byteLength/2));if(data?.type==='Buffer'&&Array.isArray(data.data)){const u=Uint8Array.from(data.data);return new Int16Array(u.buffer,u.byteOffset,Math.floor(u.byteLength/2))}return null}
function playVoiceChunk(from,raw,sampleRate=VOICE_RATE,seq=0){if(!speakerOn)return;ensureVoiceOutput();if(!audioCtx||!voiceOutput)return;const n=Number(seq)||0,last=voiceLastSeq.get(from)||0;if(n&&n<=last)return;if(n)voiceLastSeq.set(from,n);const pcm=pcmArray(raw);if(!pcm?.length)return;const buf=audioCtx.createBuffer(1,pcm.length,sampleRate||VOICE_RATE),ch=buf.getChannelData(0),fade=Math.min(32,Math.floor(pcm.length/6));for(let i=0;i<pcm.length;i++){let gain=1;if(i<fade)gain=i/fade;else if(i>=pcm.length-fade)gain=(pcm.length-1-i)/fade;ch[i]=(pcm[i]/32768)*Math.max(.08,gain)}const src=audioCtx.createBufferSource();src.buffer=buf;src.connect(voiceOutput);const now=audioCtx.currentTime,prev=voiceNextTime.get(from)||0;let start;if(prev<now-.035||prev>now+.32)start=now+.055;else start=Math.max(now+.018,prev);src.start(start);voiceNextTime.set(from,start+buf.duration);src.onended=()=>{if((voiceNextTime.get(from)||0)<audioCtx.currentTime-.18)voiceNextTime.delete(from)}}
socket.on('voice:chunk',d=>{if(d?.from&&d.from!==socket.id)playVoiceChunk(d.from,d.pcm,d.sampleRate,d.seq)});
socket.on('voice:state',({playerId,voiceOn})=>{const p=roomState?.players?.find(x=>x.id===playerId);if(p){p.voiceOn=!!voiceOn;renderPlayers()}});
$('micBtn').addEventListener('click',async()=>{ensureAudio();if(micOn)stopLocalVoice();else await startLocalVoice(false)});
$('speakerBtn').addEventListener('click',()=>{ensureAudio();speakerOn=!speakerOn;if(voiceOutput)voiceOutput.gain.value=speakerOn ? .94 : 0;renderAudioButtons()});
document.addEventListener('visibilitychange',()=>{if(!document.hidden&&audioCtx?.state==='suspended')audioCtx.resume().catch(()=>{})});
