...
fetch('/config',{cache:'no-store'}).then(r=>r.json()).then(d=>{appSettings={...appSettings,...(d.settings||{})};sfxOn=appSettings.soundDefaultOn!==false;renderAudioButtons()}).catch(()=>{});
...
function enterRoom(room,messages=[]){roomState=room;intentionalLeave=false;wasDisconnected=false;landing.classList.add('hidden');gameShell.classList.remove('hidden');visualPositions.clear();room.players.forEach(p=>visualPositions.set(p.id,p.position||1));setChatHistory(messages);renderAll();sfx('join');history.replaceState({},'',room.path||'/snakes');setTimeout(()=>startLocalVoice(true),120)}
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('/service-worker.js?v=11.0.0',{updateViaCache:'none'});await reg.update()}catch{}})}
...
const chatFab=$('chatFab'),chatCloseBtn=$('chatCloseBtn');
...
chatFab?.addEventListener('click',()=>setChatOpen(!chatIsVisible()));chatCloseBtn?.addEventListener('click',()=>setChatOpen(false));chatBackdrop?.addEventListener('click',()=>setChatOpen(false));
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&chatIsVisible())setChatOpen(false)});
...
// ---------- RELIABLE LIVE VOICE (SOCKET PCM RELAY) ----------
...
let voiceOutput=null;const voiceNextTime=new Map();const VOICE_RATE=16000;
function renderAudioButtons(){const mb=$('micBtn');mb.classList.toggle('on',micOn);mb.querySelector('b').textContent=micOn?'VOICE LIVE':'VOICE OFF';mb.title=micOn?'Your microphone is live':'Turn on live voice';$('speakerBtn').classList.toggle('on',speakerOn);$('speakerBtn').querySelector('b').textContent=speakerOn?'SPEAKER ON':'SPEAKER OFF';$('soundBtn').classList.toggle('on',sfxOn);$('soundBtn').querySelector('b').textContent=sfxOn?'SFX ON':'SFX OFF';if(voiceOutput)voiceOutput.gain.value=speakerOn?1:0}
function ensureVoiceOutput(){ensureAudio();if(!audioCtx)return null;if(!voiceOutput){voiceOutput=audioCtx.createGain();voiceOutput.gain.value=speakerOn?1:0;voiceOutput.connect(audioCtx.destination)}return voiceOutput}
function downsampleToInt16(input,inputRate,targetRate=VOICE_RATE){if(!input?.length)return new Int16Array(0);if(inputRate<=targetRate){const out=new Int16Array(input.length);for(let i=0;i<input.length;i++){const s=Math.max(-1,Math.min(1,input[i]));out[i]=s<0?s*32768:s*32767}return out}const ratio=inputRate/targetRate,outLen=Math.max(1,Math.floor(input.length/ratio)),out=new Int16Array(outLen);let pos=0;for(let i=0;i<outLen;i++){const next=Math.min(input.length,Math.floor((i+1)*ratio));let sum=0,count=0;for(;pos<next;pos++){sum+=input[pos];count++}const s=Math.max(-1,Math.min(1,count?sum/count:0));out[i]=s<0?s*32768:s*32767}return out}
function sendVoicePcm(pcm){if(!micOn||!roomState||!socket.connected||!pcm?.byteLength)return;socket.volatile.emit('voice:chunk',{pcm:pcm.buffer,sampleRate:VOICE_RATE,seq:++voiceSeq})}
async function buildVoiceCapture(){ensureAudio();if(!audioCtx||!localStream)return false;voiceSourceNode=audioCtx.createMediaStreamSource(localStream);voiceSilentGain=audioCtx.createGain();voiceSilentGain.gain.value=0;voiceSilentGain.connect(audioCtx.destination);
  if(audioCtx.audioWorklet&&window.AudioWorkletNode){try{await audioCtx.audioWorklet.addModule('/voice-worklet-v11.js?v=11.0.0');const node=new AudioWorkletNode(audioCtx,'playverse-voice-capture',{numberOfInputs:1,numberOfOutputs:1,outputChannelCount:[1]});node.port.onmessage=e=>{const pcm=e.data?.pcm;if(pcm)sendVoicePcm(new Int16Array(pcm))};voiceSourceNode.connect(node);node.connect(voiceSilentGain);voiceCaptureNode=node;return true}catch(e){console.warn('AudioWorklet unavailable, using fallback',e)}}
  const node=audioCtx.createScriptProcessor?.(4096,1,1);if(!node)return false;node.onaudioprocess=e=>sendVoicePcm(downsampleToInt16(e.inputBuffer.getChannelData(0),audioCtx.sampleRate));voiceSourceNode.connect(node);node.connect(voiceSilentGain);voiceCaptureNode=node;return true;
}
function teardownVoiceCapture(){try{voiceSourceNode?.disconnect()}catch{}try{voiceCaptureNode?.disconnect()}catch{}try{voiceSilentGain?.disconnect()}catch{}if(voiceCaptureNode&&'onaudioprocess'in voiceCaptureNode)voiceCaptureNode.onaudioprocess=null;voiceSourceNode=null;voiceCaptureNode=null;voiceSilentGain=null;voiceNextTime.clear()}
async function startLocalVoice(auto=false){if(micOn||voiceStarting)return true;if(!navigator.mediaDevices?.getUserMedia){if(!auto)toast(location.protocol==='https:'?'Voice is not supported in this browser.':'Voice needs HTTPS to use the microphone.');return false}voiceStarting=true;try{ensureAudio();localStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,channelCount:1},video:false});const ok=await buildVoiceCapture();if(!ok)throw new Error('Audio capture unavailable');micOn=true;socket.emit('voice:state',{voiceOn:true});renderAudioButtons();toast('Live voice connected.');return true}catch(e){localStream?.getTracks().forEach(t=>t.stop());localStream=null;teardownVoiceCapture();micOn=false;renderAudioButtons();if(!auto||e?.name!=='NotAllowedError')toast('Microphone blocked. Tap VOICE OFF and allow microphone.');return false}finally{voiceStarting=false}}
...
function playVoiceChunk(from,raw,sampleRate=VOICE_RATE){if(!speakerOn)return;ensureVoiceOutput();if(!audioCtx||!voiceOutput)return;const pcm=pcmArray(raw);if(!pcm?.length)return;const buf=audioCtx.createBuffer(1,pcm.length,sampleRate||VOICE_RATE),ch=buf.getChannelData(0);for(let i=0;i<pcm.length;i++)ch[i]=pcm[i]/32768;const src=audioCtx.createBufferSource();src.buffer=buf;src.connect(voiceOutput);const now=audioCtx.currentTime,prev=voiceNextTime.get(from)||0,start=prev<now-.05||prev>now+.55?now+.045:Math.max(now+.02,prev);src.start(start);voiceNextTime.set(from,start+buf.duration);src.onended=()=>{if((voiceNextTime.get(from)||0)<audioCtx.currentTime-.2)voiceNextTime.delete(from)}}
socket.on('voice:chunk',d=>{if(d?.from&&d.from!==socket.id)playVoiceChunk(d.from,d.pcm,d.sampleRate)});
...
$('speakerBtn').addEventListener('click',()=>{ensureAudio();speakerOn=!speakerOn;if(voiceOutput)voiceOutput.gain.value=speakerOn?1:0;renderAudioButtons()});$('soundBtn').addEventListener('click',()=>{ensureAudio();sfxOn=!sfxOn;renderAudioButtons();if(sfxOn)sfx('turn')});
